...
def main(args):
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            ...
            for i in range(len(models)):
                ...
                load_model = ConditionalUNet.from_pretrained(
    ...
    if args.model_config_name_or_path is None:
        ...
        config = ConditionalUNet.load_config(args.model_config_name_or_path)
        model = ConditionalUNet.from_config(config)
    ...
    for epoch in range(first_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            noise = torch.randn(
                clean_images.shape,
                dtype=(
                    torch.float32 if args.mixed_precision == "no" else torch.float16
                ),
            ...
            with accelerator.accumulate(model):
                ...
                model_output = model(noisy_images, timesteps, batch["condition"]).sample
